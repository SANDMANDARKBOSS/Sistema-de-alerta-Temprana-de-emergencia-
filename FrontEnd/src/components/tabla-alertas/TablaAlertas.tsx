...
const Cronometro = ({ start }: { start: Date }) => {
  ...
  return <span className="font-inter tabular-nums tracking-wider text-[#111827] font-semibold">{elapsed}</span>;
};
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-[#1565C0] font-bold text-sm">
                      {alerta.paciente.nombre.split(' ').map(n => n[0]).join('').substring(0, 2)}
                    </div>
                    <div>
                  ...
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      <div className={clsx(
                        ...
                        alerta.estado === 'notificada' && "bg-[#22C55E]",
                        alerta.estado === 'en-validacion' && "bg-[#F59E0B]",
                        alerta.estado === 'invalida' && "bg-[#EF4444]"
                      )}></div>
                      <span className={clsx(
                        "text-sm font-bold",
                        ...
                        {alerta.estado === 'en-validacion' ? 'En Validación' : 
                         alerta.estado === 'invalida' ? 'Póliza Inválida' : 'Notificada'}
                      </span>
                    </div>
                    <span className="text-[11px] text-[#6B7280] ml-4">{alerta.estadoSubtexto}</span>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <div className="flex flex-col">
